//Cálculo da recaracterização com quadratura de Gauss-Laguerre de Al Ajmi 2011-F1. 5 pseudocomponentes
clear
clc

exec("C:\Users\jvms9\OneDrive\Área de Trabalho\Finalização de dissertação\Comparação Al-Ajmi CBTERMO versão corrigida\Etapa 2\F1\Gama, gaussiana, 5 pseudos\Função PC-SAFT para obter Z.sce")

//Lumping através de distribuição gama e quadratura gaussiana
alpha = 0.6110237
eta = 93.268353 //eta representa a menor massa na fração C7+.
M7mais = 291
betha = 397.06443
x7mais = 0.49845
SG7mais = 0.8945/0.999

//Lumping através do método da quadratura de Gauss-Laguerre (p.185 Riazi). Parâmetros: referência Abramowitz e Stegun (1964)
Np = 5 //número de pseudocomponentes
if Np == 2 then
    y = [0.585786437627;3.414213562373]
    w = [8.53553390593e-1;1.46446609407e-1]
elseif Np == 3 then
    y = [0.415774556783;2.294280360279;6.289945082937]
    w = [7.11093009929e-1;2.78517733569e-1;1.03892565016e-2]
elseif Np == 4 then
    y = [0.322547689619;1.745761101158;4.536620296921;9.395070912301]
    w = [6.03154104342e-1;3.57418692438e-1;3.88879085150e-2;5.39294705561e-4]
elseif Np == 5 then
    y = [0.263560319718;1.413403059107;3.596425771041;7.085810005859;12.640800844276]
    w = [5.21755610583e-1;3.98666811083e-1;7.59424496817e-2;3.61175867992e-3;2.33699723858e-5]
elseif Np == 6 then
    y = [0.222846604179;1.188932101673;2.992736326059;5.775143569105;9.837467418383;15.982873980602]
    w = [4.58964673950e-1;4.17000830772e-1;1.13373382074e-1;1.03991974531e-2;2.61017202815e-4;8.98547906430e-7]
end //referência: Abramowitz and Stegun,1972

z = w.*y.^(alpha-1)./gamma(alpha) //fração molar normalizada (soma é 1)
xf = x7mais*z
M = y*betha+eta

//Cálculo dos parâmetros da PC-SAFT
//Cálculo de SG de cada pseudocomponente através da correlação de Soreide
xn = z
xn7mais = sum(z)
function z = s(Cf)
    for i = 1:length(M)
...
//Corrigindo frações negativas que ocorreram para número de carbono com M<200 (a fração negativa é descontada de todas as frações, após isso, é feita rernormalização)
PNA = [xCp,xCn,xCa]
[linh,col] = size(PNA)
for i = 1:linh
    for j = 1:col
...
//Cálculo dos parâmetros m, epsilon_K e sigma da PC-SAFT para cada SCN (LIANG et al., 2014)
for i = 1:length(SG)
